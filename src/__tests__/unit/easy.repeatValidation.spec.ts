...
describe('repeatValidation', () => {
  describe('정상 동작', () => {
    it('type="none" 때 검증을 스킵한다', () => {
      const repeat = { type: 'none' as RepeatType, interval: 1 };
      expect(validateRepeatRule(repeat)).toEqual({ valid: true });
    });
    it('count만 지정 시 정상 통과한다', () => {
      const repeat = { type: 'daily' as RepeatType, interval: 1, count: 5 };
      expect(validateRepeatRule(repeat)).toEqual({ valid: true });
    });
  ...
  describe('에러 케이스', () => {
    describe('종료 조건 검증', () => {
      it('count와 until을 모두 누락하면 에러를 발생시킨다', () => {
        const repeat = { type: 'daily' as RepeatType, interval: 1 };
    ...
    describe('값 범위 검증', () => {
      it('count가 1보다 작으면 에러를 발생시킨다', () => {
        const repeat = { type: 'daily' as RepeatType, interval: 1, count: 0 };
        expect(validateRepeatRule(repeat)).toEqual({
          valid: false,
      ...
      it('count가 1000을 초과하면 에러를 발생시킨다', () => {
        const repeat = { type: 'daily' as RepeatType, interval: 1, count: 1001 };
        expect(validateRepeatRule(repeat)).toEqual({
          valid: false,
          code: 'INVALID_COUNT_RANGE',
          message: 'count는 1~1000 사이여야 합니다.',
      ...
      it('interval이 1보다 작으면 에러를 발생시킨다', () => {
        const repeat = { type: 'daily' as RepeatType, interval: 0, count: 5 };
        expect(validateRepeatRule(repeat)).toEqual({
          valid: false,
      ...
      it('interval이 12를 초과하면 에러를 발생시킨다', () => {
        const repeat = { type: 'daily' as RepeatType, interval: 13, count: 5 };
        expect(validateRepeatRule(repeat)).toEqual({
          valid: false,
          code: 'INVALID_INTERVAL_RANGE',
          message: 'interval은 1~12 사이여야 합니다.',
