...
**파일**: `__tests__/unit/medium.repeatRuleGenerator.spec.ts`
...
**파일**: `__tests__/unit/easy.repeatValidation.spec.ts`
...
**파일**: `__tests__/unit/easy.dateTimeUtils.spec.ts`
...
**파일**: `__tests__/unit/easy.timeAdjustment.spec.ts`
...
**파일**: `__tests__/medium.recurringEvent.integration.spec.tsx`
...
http.post('/api/events', async ({ request }) => {
  ...
  if (body.repeat?.enabled && !body.repeat?.count && !body.repeat?.endDate) {
    return HttpResponse.json(
      {
        success: false,
        error: {
