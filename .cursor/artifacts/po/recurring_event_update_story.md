...
1. 사용자가 반복 이벤트에서 "편집"을 클릭합니다 (예: 2025-11-05의 "주간 팀 회의", 매주 반복)
2. 시스템이 반복 이벤트임을 감지하고 확인 다이얼로그를 표시합니다: **"이 인스턴스만 수정하시겠습니까?"**
3. 사용자 선택:
   - **"예"**: 
     - 선택한 반복 이벤트 인스턴스가 삭제됨
     - 새 UUID로 새로운 단일 이벤트가 생성됨 (`repeat.type = 'none'`, 반복 아이콘 제거)
     - 남은 반복 이벤트는 유지되며 반복 아이콘도 유지됨
   - **"아니오"**: 
     - 전체 시리즈 수정 → 시리즈의 모든 인스턴스 수정, 모든 인스턴스의 반복 아이콘 유지
4. 시스템이 선택된 작업을 수행하고 그에 따라 캘린더 뷰를 업데이트합니다
...
And "예"를 클릭하면
Then 선택한 반복 이벤트 인스턴스가 삭제되고
And 새 UUID로 새로운 단일 이벤트가 생성되고
And 새 이벤트는 repeat.type = 'none'을 가지며
And 새 이벤트는 반복 아이콘을 표시하지 않으며
And 남은 반복 이벤트는 유지되며 반복 아이콘도 유지되고
And "일정이 수정되었습니다."라는 성공 메시지가 표시된다
```

### 시나리오 2: 전체 시리즈 수정 ("아니오" 선택)
...
- **MSW 핸들러 설정**
  ...
  - `DELETE /api/events/:id` 엔드포인트 모의 (선택한 인스턴스 삭제용)
  - 성공 시나리오 (200, 201 응답)
  - 오류 시나리오 (400, 404, 500)
  - 네트워크 오류 시뮬레이션

- **테스트 유틸리티 함수**
  - `repeat.id`가 있는 `createMockRecurringEvent()` 헬퍼
  - `createMockEventSeries()` 헬퍼 (같은 repeat.id를 가진 여러 이벤트)
  - `setupRecurringEventUpdateTests()` 헬퍼

- **모의 데이터**
  - `repeat.id`가 있는 주간 패턴 반복 이벤트
...
#### 2.1 단위 테스트
- **다이얼로그 컴포넌트 테스트**
  - `RecurringEventUpdateDialog`가 이벤트가 반복일 때 렌더링됨
...
- **수정 로직 테스트**
  ...
  - `updateSingleInstance()`가 선택한 원본 이벤트를 삭제함 (DELETE /api/events/:id)
  - `updateSingleInstance()`가 남은 반복 이벤트를 유지함
...
- **아이콘 표시 로직 테스트**
  ...
  - `RecurringBadge`가 이벤트가 반복일 때 렌더링됨
  - `RecurringBadge`가 이벤트가 반복이 아닐 때 렌더링되지 않음

#### 2.2 통합 테스트
- **이벤트 편집 플로우 테스트**
  - 반복 이벤트에서 "편집" 클릭 시 다이얼로그가 열림
  - "예" 선택 시 단일 인스턴스를 수정하고 아이콘을 제거함
  - "예" 선택 시 선택한 인스턴스가 삭제되고 새 단일 이벤트가 생성됨
  - "예" 선택 시 남은 반복 이벤트는 유지되고 반복 아이콘도 유지됨
...
- **API 통합 테스트**
  ...
  - 단일 인스턴스 수정 시 DELETE /api/events/:id 호출 (선택한 인스턴스 삭제)
...
- **updateSingleInstance() 함수**
  ...
  - DELETE /api/events/:id 호출하여 선택한 원본 이벤트 삭제
  - 남은 반복 이벤트는 유지됨 (시리즈의 다른 인스턴스 보존)
  - 수정된 이벤트 반환
  - 성공 메시지 표시: "일정이 수정되었습니다."
...
- **단일 인스턴스 수정 API**
  ...
  - DELETE /api/events/:id 엔드포인트 사용 (선택한 인스턴스 삭제)
  - 응답 처리
  - 오류 처리
...
- **API 클라이언트 함수**
  ...
  - 단일 인스턴스용 deleteEvent(eventId) (DELETE /api/events/:id)
...
- 다이얼로그 로직을 커스텀 훅 `useRecurringEventUpdateDialog()`로 추출
  - 다이얼로그 상태 관리
  - 핸들러 함수
- 수정 함수를 유틸리티 모듈 `recurringEventUtils.ts`로 분리
  - updateSingleInstance() 함수
  - updateRecurringSeries() 함수
  - repeat.id 추출을 위한 헬퍼 함수
- API 호출을 서비스 모듈 `eventApiService.ts`로 추출
  - 단일 인스턴스용 createEvent()
  - 단일 인스턴스용 deleteEvent()
...
- API 엔드포인트 문서화
  ...
  - DELETE /api/events/:id (선택한 인스턴스 삭제)
...
- [ ] 선택한 인스턴스 삭제가 올바르게 작동함
- [ ] 남은 반복 이벤트 유지가 올바르게 작동함
- [ ] 오류 처리 테스트 완료 및 작동 중
- [ ] 한글 텍스트가 올바르게 표시됨
- [ ] UUID 생성이 올바르게 작동함
