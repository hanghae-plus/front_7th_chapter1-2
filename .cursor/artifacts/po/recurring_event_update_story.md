...
사용자 여정
사용자가 반복 이벤트에서 "편집"을 클릭합니다 (예: 2025-11-05의 "주간 팀 회의", 매일 반복, 2025년 12월 5일까지)
시스템이 반복 이벤트임을 감지하고 확인 다이얼로그를 표시합니다: "이 인스턴스만 수정하시겠습니까?"
사용자 선택:
"예": A = 캘린더에서 특정 날짜에 있는 이벤트를 선택했다면 해당 날짜만 반복 이벤트에서 삭제되고,  → 새 ID로 새로운 단일 이벤트 생성, 반복 아이콘 제거
B = 일정 검색에서 수정을 선택했다면 반복 이벤트의 첫 번째 이벤트만 삭제되고, 남은 반복 이벤트는 유지 → 새 ID로 새로운 단일 이벤트 생성(반복 아이콘 제거)
남아있는 반복 이벤트에는 여전히 반복 아이콘이 존재
