...
2. 반복 유형을 선택하면 (daily/weekly/monthly/yearly) 반복이 활성화된다.
3. 종료 조건을 입력한다 (count 또는 until 중 하나)
4. 고급 옵션으로 interval을 설정한다 (선택, 기본값 1)
5. 시스템이 입력값을 검증하고 반복 인스턴스를 미리 계산한다
6. 사용자가 저장하면 규칙에 따라 모든 인스턴스가 생성/갱신된다
7. 캘린더 뷰에 각 인스턴스가 표시된다
...
```gherkin
Given 반복 설정이 비활성화되어 있다 (repeat.type = 'none')
  And 반복 유형은 none 이다
When 일정을 저장한다
