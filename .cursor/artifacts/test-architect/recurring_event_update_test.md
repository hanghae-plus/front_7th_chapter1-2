...
선택한 반복 이벤트 인스턴스가 삭제되고 새 단일 이벤트가 생성됨 확인
남은 반복 이벤트는 유지되며 반복 아이콘도 유지됨 확인
...
describe('반복 이벤트 수정 - 통합 테스트', () => {
  ...
  describe('단일 인스턴스 수정 (예 선택)', () => {
    it('"예" 선택 시 선택한 반복 이벤트 인스턴스가 삭제되고 새 단일 이벤트가 생성됨', () => {
      // Developer가 구현
      // - POST /api/events 호출 (새 단일 이벤트 생성, repeat.type='none')
      // - DELETE /api/events/:id 호출 (선택한 반복 이벤트 인스턴스 삭제)
      // - 남은 반복 이벤트는 유지되고 반복 아이콘도 유지됨
    });
    
    it('선택한 인스턴스가 삭제되고 새 단일 이벤트만 생성되며 남은 반복 이벤트와 다른 인스턴스는 유지됨', () => {
      // Developer가 구현
      // - 선택한 반복 이벤트 인스턴스는 삭제됨
      // - 새 단일 이벤트(repeat.type='none', 반복 아이콘 제거)가 생성됨
      // - 남은 반복 이벤트는 유지되고 반복 아이콘도 유지됨
      // - 시리즈의 다른 인스턴스는 유지됨
    });
  });
