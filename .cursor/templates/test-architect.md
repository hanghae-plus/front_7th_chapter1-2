...
- Custom Hook: `src/__tests__/hooks/[hookName].hook.spec.ts`
- Utility: `src/__tests__/utils/[utilName].util.spec.ts`

### 기본 구조

```typescript
// 파일: src/__tests__/utils/dateUtils.util.spec.ts
...
```typescript
// 파일: src/__tests__/hooks/useCustomHook.hook.spec.ts
...
```typescript
// 파일: src/__tests__/utils/dateFormat.util.spec.ts
...
**파일 경로**: `src/__tests__/components/[ComponentName].component.spec.ts`

### 기본 구조

```typescript
// 파일: src/__tests__/components/ComponentName.component.spec.ts
...
```typescript
// 파일: src/__tests__/components/EventForm.component.spec.ts
import { describe, it } from 'vitest';
...
```typescript
// 파일: src/__tests__/components/Calendar.component.spec.ts
import { describe, it } from 'vitest';
import { render } from '@testing-library/react';
...
**파일 경로**: `src/__tests__/[featureName].spec.ts` (루트에 위치)

### 기본 구조

```typescript
// 파일: src/__tests__/eventManagement.spec.ts
import { describe, it, expect, beforeEach } from 'vitest';
...
```typescript
// 파일: src/__tests__/errorHandling.spec.ts
...
**파일 경로**: `src/__tests__/[scenarioName].spec.ts` (루트에 위치)

### 기본 구조

```typescript
// 파일: src/__tests__/userJourney.spec.ts
import { describe, it, expect, beforeEach } from 'vitest';
import { render, screen, waitFor } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { http, HttpResponse } from 'msw';
...
export const handlers = [
  ...
  http.delete('/api/events/:id', ({ params }) => {
    ...
    // 존재하지 않는 ID 시뮬레이션
    if (Number(id) === 999) {
      return HttpResponse.json({ error: '일정을 찾을 수 없습니다' }, { status: 404 });
    }
...
export const dynamicHandlers = [
  ...
  http.delete('/api/events/:id', ({ params }) => {
    const { id } = params;
    mockDatabase.events = mockDatabase.events.filter((e) => e.id !== Number(id));
    return new HttpResponse(null, { status: 204 });
  }),
];
...
**파일 경로**: `src/__tests__/components/[ComponentName].a11y.spec.ts` 또는 `src/__tests__/accessibility.spec.ts`

```typescript
// 파일: src/__tests__/components/Calendar.a11y.spec.ts
import { describe, it, expect } from 'vitest';
import { render, screen } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
...
- v2.1.0: 모든 템플릿에 파일 경로 예시 추가
  - `src/__tests__/[카테고리]/[기능명].[타입].spec.ts` 형식
  - 카테고리별 파일 위치 명시
